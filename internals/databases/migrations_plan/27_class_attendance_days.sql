...
CREATE TABLE IF NOT EXISTS class_attendance_day_stats (
  ...
  class_attendance_day_stats_school_id       UUID NOT NULL
...
-- unik per (school, kelas, tanggal) yg masih hidup
CREATE UNIQUE INDEX IF NOT EXISTS uq_cads_alive
  ON class_attendance_day_stats(
    class_attendance_day_stats_school_id,
...
CREATE INDEX IF NOT EXISTS idx_cads_school_date_alive
  ON class_attendance_day_stats(class_attendance_day_stats_school_id, class_attendance_day_stats_date)
  WHERE class_attendance_day_stats_deleted_at IS NULL;
...
SELECT
  class_attendance_day_stats_school_id       AS school_id,
...
GROUP BY school_id, class_section_id, date_trunc('week', date);
...
SELECT
  ...
  SUM(unexcused)      AS unexcused
FROM v_class_attendance_day_stats
GROUP BY school_id, class_section_id, date_trunc('month', date);
...
BEGIN
  ...
  GROUP BY p_school_id, p_class_section_id, p_start, p_end;
END;
