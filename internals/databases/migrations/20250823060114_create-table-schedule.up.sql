...
-- =========================================================
-- UNIQUE (id, school) untuk class_schedules
-- =========================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_class_schedules_id_school'
  ) THEN
...
-- =========================================================
-- UNIQUE (id, school) untuk CSST
-- =========================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_csst_id_school'
  ) THEN
...
CREATE TABLE IF NOT EXISTS class_schedule_rules (
  ...
  -- pola mingguan
  class_schedule_rule_day_of_week INT  NOT NULL
    CHECK (class_schedule_rule_day_of_week BETWEEN 1 AND 7),
  class_schedule_rule_start_time  TIME NOT NULL,
  class_schedule_rule_end_time    TIME NOT NULL
    CHECK (class_schedule_rule_end_time > class_schedule_rule_start_time),
  ...
  -- CSST ASSIGNMENT (no cache, join langsung ke CSST)
  ...
  -- waktu dalam menit untuk exclusion check
...
-- =========================================================
-- FK ke CSST (single column PK)
-- =========================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_csr_csst_singlecol'
  ) THEN
...
-- Prevent duplicate slot per schedule + CSST + hari + jam
...
-- Prevent overlap per CSST per day (pakai gist + start_min/end_min)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ex_csr_no_overlap_per_csst'
  ) THEN
    ALTER TABLE class_schedule_rules
      ...
      EXCLUDE USING gist (
        ...
        int4range(
          class_schedule_rule_start_min,
          class_schedule_rule_end_min,
          '[]'
        ) WITH &&
      )
      WHERE (class_schedule_rule_deleted_at IS NULL);
