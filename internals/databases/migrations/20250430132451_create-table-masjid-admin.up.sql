...
CREATE TABLE IF NOT EXISTS masjid_admins (
  ...
  masjid_admins_is_active BOOLEAN NOT NULL DEFAULT TRUE,

  -- timestamps explicit
  masjid_admin_created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  masjid_admin_updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  masjid_admin_deleted_at TIMESTAMP NULL
);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_masjid_admins_masjid_user_alive
  ON masjid_admins (masjid_admins_masjid_id, masjid_admins_user_id)
...
CREATE INDEX IF NOT EXISTS idx_masjid_admins_user_active_alive
  ON masjid_admins (masjid_admins_user_id)
  WHERE masjid_admins_is_active = TRUE AND masjid_admin_deleted_at IS NULL;

-- by masjid & active
CREATE INDEX IF NOT EXISTS idx_masjid_admins_masjid_active_alive
  ON masjid_admins (masjid_admins_masjid_id)
  WHERE masjid_admins_is_active = TRUE AND masjid_admin_deleted_at IS NULL;
...
CREATE TABLE IF NOT EXISTS masjid_teachers (
  ...
  masjid_teachers_created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  masjid_teachers_updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  masjid_teachers_deleted_at TIMESTAMP NULL
);
