...
DROP INDEX IF EXISTS ix_assessment_urls_by_school_live;
...
DROP INDEX IF EXISTS idx_assessments_school_created_at;
...
DROP INDEX IF EXISTS idx_assessment_types_school_active;
DROP INDEX IF EXISTS uq_assessment_types_key_per_school_alive;
...
DO $$
BEGIN
  -- Cek apakah ada constraint bernama uq_csst_id_school pada table CSST
  IF EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'uq_csst_id_school'
      AND conrelid = 'class_section_subject_teachers'::regclass
  ) THEN
    ALTER TABLE class_section_subject_teachers
      DROP CONSTRAINT uq_csst_id_school;
  ...
  ELSIF EXISTS (
    ...
    WHERE schemaname = ANY(current_schemas(true))
      ...
      AND indexname  = 'uq_csst_id_school'
  ) THEN
    DROP INDEX IF EXISTS uq_csst_id_school;
  END IF;
END$$;
