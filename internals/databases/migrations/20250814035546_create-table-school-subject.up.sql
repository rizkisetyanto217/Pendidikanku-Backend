...
-- FK komposit tenant-safe untuk SECTION (index dulu, FK di atas single, ini hanya bantu lookup)
CREATE INDEX IF NOT EXISTS idx_csst_section_masjid_alive
  ON class_section_subject_teachers (class_section_subject_teachers_section_id, class_section_subject_teachers_masjid_id)
  WHERE class_section_subject_teachers_deleted_at IS NULL;

-- Satu guru per (class_subjects) saat aktif
CREATE UNIQUE INDEX IF NOT EXISTS uq_csst_active_by_cs_alive
