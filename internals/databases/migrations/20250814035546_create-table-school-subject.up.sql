...
-- Relasi ke class_section_subject_teachers
-- =========================================
BEGIN;
...
CREATE TABLE IF NOT EXISTS user_class_subjects (
  ...
  -- konteks pengajar-mapel di kelas (wajib)
  -- (tetap pakai nama kolom lama untuk kompatibilitas,
  --  tapi FK diarahkan ke class_section_subject_teachers)
...
-- Satu baris aktif per (siswa × pengajar-mapel-kelas) — soft-delete aware
-- (tetap pakai nama kolom lama di index key)
...
-- Indeks bantu umum (rename agar lebih jelas konteksnya)
DROP INDEX IF EXISTS idx_user_class_subject_cs_alive;
CREATE INDEX IF NOT EXISTS idx_user_class_subject_csst_alive
...
-- (Opsional) Index untuk kueri laporan absensi cepat per pengajar-mapel-kelas
DROP INDEX IF EXISTS idx_user_class_subject_masjid_subject_attendance;
CREATE INDEX IF NOT EXISTS idx_user_class_subject_masjid_csst_attendance
  ON user_class_subjects(
