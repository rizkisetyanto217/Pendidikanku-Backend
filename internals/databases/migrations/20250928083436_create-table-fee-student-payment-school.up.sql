...
CREATE INDEX IF NOT EXISTS idx_fee_category_masjid_active
  ON fee_categories (fee_category_masjid_id) WHERE fee_category_is_active = TRUE;

-- =========================================
-- TABLE 2: school_fee_settings  (dengan *_snapshot)
-- =========================================
...
CREATE INDEX IF NOT EXISTS idx_fee_setting_masjid
  ON school_fee_settings(fee_setting_masjid_id);
...
-- =========================================
-- TABLE 3: student_bills (dengan *_snapshot)
-- =========================================
...
CREATE INDEX IF NOT EXISTS idx_student_bill_user    ON student_bills(student_bill_user_class_section_id);
CREATE INDEX IF NOT EXISTS idx_student_bill_period  ON student_bills(student_bill_period);
CREATE INDEX IF NOT EXISTS idx_student_bill_masjid  ON student_bills(student_bill_masjid_id);
...
CREATE INDEX IF NOT EXISTS idx_bill_yyyymm_masjid
  ON student_bills (student_bill_masjid_id, student_bill_period_yyyymm);

-- =========================================
-- TABLE 4: student_payments (manual ATAU gateway)
-- =========================================
CREATE TABLE IF NOT EXISTS student_payments (
  ...
  -- metode high-level (cash, transfer_internal, bank_transfer, qris, gopay, credit_card, dll)
  student_payment_method TEXT,

  -- ================= MANUAL FIELDS (opsional) =================
  student_payment_manual_receipt_number   VARCHAR(64),     -- nomor kwitansi manual
  student_payment_manual_channel          TEXT,            -- cash / transfer_internal / dll
  student_payment_manual_reference_number VARCHAR(64),     -- no. referensi internal/bukti
  student_payment_manual_received_by      VARCHAR(120),    -- petugas kasir
  student_payment_manual_received_at      TIMESTAMPTZ,     -- waktu diterima

  -- ================= PROVIDER/GATEWAY FIELDS (opsional, untuk Midtrans) =================
  ...
  student_payment_provider_payment_type         TEXT,        -- bank_transfer / qris / gopay / credit_card / ...
...
-- Indeks umum
CREATE INDEX IF NOT EXISTS idx_student_payment_bill
  ON student_payments(student_payment_bill_id);
CREATE INDEX IF NOT EXISTS idx_student_payment_masjid
  ON student_payments(student_payment_masjid_id);
CREATE INDEX IF NOT EXISTS brin_payment_paid_at
  ON student_payments USING BRIN (student_payment_paid_at);

-- Anti duplikasi settlement gateway (kalau via Midtrans)
...
-- (Opsional) Guard soft: minimal salah satu cara terisi (manual atau provider)
-- -> Biarkan di-app layer agar fleksibel, tidak dibuat CHECK di DB.
