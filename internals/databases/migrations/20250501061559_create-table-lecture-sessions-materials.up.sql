CREATE TABLE IF NOT EXISTS lecture_sessions_materials (
  ...
  -- ✅ Tambahan masjid_id
  lecture_sessions_material_masjid_id UUID NOT NULL REFERENCES masjids(masjid_id) ON DELETE CASCADE,

  lecture_sessions_material_created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Indexing
CREATE INDEX IF NOT EXISTS idx_lecture_sessions_materials_lecture_session_id 
  ON lecture_sessions_materials(lecture_sessions_material_lecture_session_id);

CREATE INDEX IF NOT EXISTS idx_lecture_sessions_materials_masjid_id 
  ON lecture_sessions_materials(lecture_sessions_material_masjid_id);
...
-- Indexing
CREATE INDEX IF NOT EXISTS idx_lecture_sessions_assets_lecture_session_id 
  ON lecture_sessions_assets(lecture_sessions_asset_lecture_session_id);
...
CREATE INDEX IF NOT EXISTS idx_lecture_sessions_assets_masjid_id 
  ON lecture_sessions_assets(lecture_sessions_asset_masjid_id);

CREATE INDEX IF NOT EXISTS idx_lecture_sessions_assets_created_at 
  ON lecture_sessions_assets(lecture_sessions_asset_created_at);
