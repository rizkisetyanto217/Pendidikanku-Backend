...
// === NEW: parse UUID slice langsung dari "any" (klaim JWT) ===
func parseUUIDSliceFromAny(v any, key string) ([]uuid.UUID, error) {
	raw := normalizeLocalsToStrings(v)
	if len(raw) == 0 {
		return nil, fiber.NewError(fiber.StatusUnauthorized, key+" kosong di token")
	}
...
func readJWTClaims(c *fiber.Ctx) map[string]any {
	...
	// Optional: fallback header custom atau cookie TIDAK diprioritaskan
	return nil
}
...
// --- masjid_roles parsing (CLAIMS FIRST) ---

func parseMasjidRoles(c *fiber.Ctx) ([]MasjidRolesEntry, error) {
	// ⬅️ PENTING: ambil dari klaim JWT LEBIH DULU supaya tidak "terscope" oleh middleware.
...
func IsDKM(c *fiber.Ctx) bool {
	...
	if roleExistsInAnyMasjid(c, "dkm") {
		return true
	}
...
func IsStudent(c *fiber.Ctx) bool {
	...
	if roleExistsInAnyMasjid(c, "student") {
		return true
	}
...
func parsePairsFromLocals(c *fiber.Ctx, localsKey, masjidKey, secondKey string) ([]recordPair, error) {
	// Prefer klaim JWT
	if any := claimAny(readJWTClaims(c), localsKey); any != nil {
		c.Locals(localsKey, any)
	}
	...
	case []map[string]any:
		...
		for _, m := range t {
			if rp, ok := drainPair(m, masjidKey, secondKey); ok {
				out = append(out, rp)
			}
		}
		if len(out) == 0 {
...
func parseTeacherRecordsFromLocals(c *fiber.Ctx) ([]TeacherRecordEntry, error) {
	...
	for _, p := range pairs {
		out = append(out, TeacherRecordEntry{MasjidTeacherID: p.SecondID, MasjidID: p.MasjidID})
	}
	return out, nil
}
...
func parseStudentRecordsFromLocals(c *fiber.Ctx) ([]StudentRecordEntry, error) {
	...
	for _, p := range pairs {
		out = append(out, StudentRecordEntry{MasjidStudentID: p.SecondID, MasjidID: p.MasjidID})
	}
	return out, nil
}
...
/* ============================================
   Multi-tenant getter (CLAIMS FIRST untuk masjid_ids)
   ============================================ */

func GetMasjidIDsFromToken(c *fiber.Ctx) ([]uuid.UUID, error) {
	// 1) langsung dari locals (kalau middleware sudah set)
	if ids, err := parseUUIDSliceFromLocals(c, LocMasjidIDs); err == nil && len(ids) > 0 {
		return ids, nil
	}
	// 2) dari klaim JWT: "masjid_ids": [ "...", ... ]
	if any := claimAny(readJWTClaims(c), "masjid_ids"); any != nil {
		if ids, err := parseUUIDSliceFromAny(any, "masjid_ids"); err == nil && len(ids) > 0 {
			// cache ke locals biar cepat
			c.Locals(LocMasjidIDs, normalizeLocalsToStrings(any))
			return ids, nil
		}
	}

	// 3) teacher_records → daftar masjid
	if recs, err := parseTeacherRecordsFromLocals(c); err == nil && len(recs) > 0 {
		seen := map[uuid.UUID]struct{}{}
		out := make([]uuid.UUID, 0, len(recs))
		for _, r := range recs {
	...
	// 4) student_records → daftar masjid
	if recs, err := parseStudentRecordsFromLocals(c); err == nil && len(recs) > 0 {
		seen := map[uuid.UUID]struct{}{}
		out := make([]uuid.UUID, 0, len(recs))
		for _, r := range recs {
	...
	// 5) fallback dari masjid_roles terstruktur
	if ids, err := getMasjidIDsFromMasjidRoles(c); err == nil && len(ids) > 0 {
		return ids, nil
	}

	// 6) terakhir: kumpulkan dari locals legacy
