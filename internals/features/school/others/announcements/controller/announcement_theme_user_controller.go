...
/*
Refactor note:
- Semua entitas URL (AnnouncementURL*) dihapus dari flow.
...
func toAnnouncementEmbeds(
	...
	for i := range src {
		...
		// convert gorm.DeletedAt -> *time.Time (DTO pakai *time.Time)
...
func (h *AnnouncementThemeController) List(c *fiber.Ctx) error {
	...
	if idStr != "" {
		...
		if incAnn {
			annsByTheme, err := h.fetchAnnouncementsForThemes(
				h.DB, masjidID, []uuid.UUID{one.AnnouncementThemesID},
				annActiveOnly, annSectionID, annDateFrom, annDateTo,
				annSortBy, annOrder, annLimitPerTheme,
			)
			if err != nil {
				return helper.JsonError(c, fiber.StatusInternalServerError, err.Error())
			}
			resp.Announcements = toAnnouncementEmbeds(annsByTheme[one.AnnouncementThemesID])
		}
	...
	if incAnn && len(rows) > 0 {
		...
		for themeID, list := range annsByTheme {
			if pos, ok := indexByID[themeID]; ok && pos >= 0 && pos < len(resp) {
				resp[pos].Announcements = toAnnouncementEmbeds(list)
			}
		}
	}
