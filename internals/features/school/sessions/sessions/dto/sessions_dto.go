...
/*
	=========================================================
	  URL DTO (opsional, tabel terpisah)
...
type ClassAttendanceSessionURL struct {
	...
	// Properti konten
	ClassAttendanceSessionURLKind         string  `json:"class_attendance_session_url_kind"` // banner,image,video,attachment,link
...
/*
List query:
- Filter utama: teacher/schedule/date range/keyword
- Filter turunan (section/subject) via JOIN opsional (di repo/controller)
*/
...
type ClassAttendanceSessionResponse struct {
	...
	ClassAttendanceSessionTeacherName  *string    `json:"class_attendance_session_teacher_name,omitempty"`  // enrichment
	ClassAttendanceSessionTeacherEmail *string    `json:"class_attendance_session_teacher_email,omitempty"` // enrichment

	// Rekap kehadiran (nullable di DB → pointer di DTO)
...
func (r CreateClassAttendanceSessionRequest) ToModel() attendanceModel.ClassAttendanceSessionModel {
	return attendanceModel.ClassAttendanceSessionModel{
		...
		ClassAttendanceSessionDate:        r.ClassAttendanceSessionDate, // pointer → pakai DEFAULT CURRENT_DATE kalau nil
...
func FromClassAttendanceSessionModel(m attendanceModel.ClassAttendanceSessionModel) ClassAttendanceSessionResponse {
	...
	// date: *time.Time → time.Time (zero jika nil)
	var date time.Time
