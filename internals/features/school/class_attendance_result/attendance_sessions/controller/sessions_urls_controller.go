...
import (
	...
	sessiondto "masjidku_backend/internals/features/school/class_attendance_result/attendance_sessions/dto"
	sessionmodel "masjidku_backend/internals/features/school/class_attendance_result/attendance_sessions/model"
...
func (ctl *ClassAttendanceSessionURLController) Create(c *fiber.Ctx) error {
	...
	if isMultipart {
		sessIDStr := strings.TrimSpace(c.FormValue("class_attendance_session_url_session_id"))
		if sessIDStr == "" {
			return helper.JsonError(c, fiber.StatusBadRequest, "class_attendance_session_url_session_id wajib diisi")
		}
		sessID, perr := uuid.Parse(sessIDStr)
		if perr != nil {
			return helper.JsonError(c, fiber.StatusBadRequest, "class_attendance_session_url_session_id tidak valid")
		}
		...
		if ferr == nil && fh != nil && fh.Size > 0 {
			// pakai helper wrapper (konversi → webp + path standar)
			...
			h := strings.TrimSpace(c.FormValue("class_attendance_session_url_href"))
			if h == "" {
				return helper.JsonError(c, fiber.StatusBadRequest, "Wajib mengirim file atau class_attendance_session_url_href")
			}
			href = h
		}
		...
		mdl := sessionmodel.ClassAttendanceSessionURLModel{
			...
			ClassAttendanceSessionURLLabel:     lbl,
	...
	// JSON mode (tanpa file) — tetap sama
	...
	if err := ctl.DB.
		Raw(`SELECT EXISTS(
				...
				WHERE class_attendance_sessions_id = ?
				  AND class_attendance_sessions_masjid_id = ?
				  AND class_attendance_sessions_deleted_at IS NULL
			)`, req.ClassAttendanceSessionURLSessionID, masjidID).
		Scan(&ok).Error; err != nil {
		return helper.JsonError(c, fiber.StatusInternalServerError, "Gagal memeriksa sesi")
	}
	if !ok {
