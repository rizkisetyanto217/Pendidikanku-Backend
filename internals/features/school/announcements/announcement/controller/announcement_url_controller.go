...
func (ctl *AnnouncementURLController) Create(c *fiber.Ctx) error {
	...
	if isMultipart {
		...
		if annIDStr == "" {
			return helper.JsonError(c, fiber.StatusBadRequest, "announcement_url_announcement_id wajib diisi")
		}
		announcementID, perr := uuid.Parse(annIDStr)
		if perr != nil {
			return helper.JsonError(c, fiber.StatusBadRequest, "announcement_url_announcement_id tidak valid (uuid)")
		}

		// Siapkan OSS sekali
		svc, err := helperOSS.NewOSSServiceFromEnv("")
		if err != nil {
			return helper.JsonError(c, fiber.StatusBadGateway, fmt.Sprintf("OSS init: %v", err))
		}
		...
		if len(files) > 0 {
			...
			for i, fh := range files {
				if fh == nil || fh.Size == 0 {
					return helper.JsonError(c, fiber.StatusBadRequest, "Salah satu file kosong")
				}
			...
			return helper.JsonCreated(c, "Berhasil upload banyak announcement URL", results)
		}
		...
		if fh, ferr := c.FormFile("file"); ferr == nil && fh != nil && fh.Size > 0 {
			...
			return helper.JsonCreated(c, "Berhasil membuat announcement URL", resp)
		}

		// Terakhir: tanpa file → pakai href manual (single)
		href := strings.TrimSpace(c.FormValue("announcement_url_href"))
		if href == "" {
			return helper.JsonError(c, fiber.StatusBadRequest, "Harus unggah file (file/files[]) atau isi announcement_url_href")
		}
		var label *string
		if lbl := strings.TrimSpace(c.FormValue("announcement_url_label")); lbl != "" {
			l := lbl
	...
	if href == "" {
		return helper.JsonError(c, fiber.StatusBadRequest, "URL tidak boleh kosong")
	}
...
func (ctl *AnnouncementURLController) Restore(c *fiber.Ctx) error {
	...
	return helper.JsonOK(c, "Berhasil restore", fiber.Map{
		"id": id.String(),
	})
}
