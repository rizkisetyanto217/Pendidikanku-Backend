...
// Legacy: filter berdasarkan users.id guru → join ke masjid_teachers
func scopeTeacherUserId(teacherUserId *uuid.UUID) func(*gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		...
		`).Where("mt.masjid_teachers_user_id = ?", *teacherUserId)
	}
}
...
func (ctrl *ClassAttendanceSessionController) isTeacherOfSession(session attendanceModel.ClassAttendanceSessionModel, userID, masjidID uuid.UUID) (bool, error) {
	...
	q := ctrl.DB.Table("masjid_teachers AS mt").
		...
		Where("mt.masjid_teachers_user_id = ?", userID).
		Where("mt.masjid_teachers_masjid_id = ?", masjidID)
...
func (ctrl *ClassAttendanceSessionController) ListClassAttendanceSessions(c *fiber.Ctx) error {
	...
	if teacherUserIDPtr != nil {
		...
		`).Where("mt_q.masjid_teachers_user_id = ?", *teacherUserIDPtr)
	}
