...
/* =========================================================
   DBTIME / SCHOOL TIME NORMALIZATION HELPERS
========================================================= */

// semua time.Time di-resolve ke "school time" berdasarkan context
func normalizeStudentClassSectionTimesToSchoolTime(c *fiber.Ctx, r *StudentClassSectionResp) {
	r.StudentClassSectionAssignedAt = dbtime.ToSchoolTime(c, r.StudentClassSectionAssignedAt)
	r.StudentClassSectionUnassignedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionUnassignedAt)
	r.StudentClassSectionCompletedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionCompletedAt)

	r.StudentClassSectionGradedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionGradedAt)
	r.StudentClassSectionStudentNotesUpdatedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionStudentNotesUpdatedAt)
	r.StudentClassSectionHomeroomNotesUpdatedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionHomeroomNotesUpdatedAt)

	r.StudentClassSectionCreatedAt = dbtime.ToSchoolTime(c, r.StudentClassSectionCreatedAt)
	r.StudentClassSectionUpdatedAt = dbtime.ToSchoolTime(c, r.StudentClassSectionUpdatedAt)
	r.StudentClassSectionDeletedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionDeletedAt)
}

func normalizeStudentClassSectionCompactTimesToSchoolTime(c *fiber.Ctx, r *StudentClassSectionCompactResp) {
	r.StudentClassSectionAssignedAt = dbtime.ToSchoolTime(c, r.StudentClassSectionAssignedAt)
	r.StudentClassSectionUnassignedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionUnassignedAt)
	r.StudentClassSectionCompletedAt = dbtime.ToSchoolTimePtr(c, r.StudentClassSectionCompletedAt)
...
/* =========================================================
   VARIAN: WITH SCHOOL TIME (dipakai controller)
========================================================= */

// Full response, 1 row
func FromModelWithSchoolTime(c *fiber.Ctx, m *model.StudentClassSection) StudentClassSectionResp {
	resp := FromModel(m)
	normalizeStudentClassSectionTimesToSchoolTime(c, &resp)
	return resp
}

// Full response, slice
func FromModelsStudentClassSectionWithSchoolTime(c *fiber.Ctx, list []model.StudentClassSection) []StudentClassSectionResp {
	out := make([]StudentClassSectionResp, 0, len(list))
	for i := range list {
		r := FromModel(&list[i])
		normalizeStudentClassSectionTimesToSchoolTime(c, &r)
		out = append(out, r)
	}
	return out
}
...
// Compact, 1 row
func FromModelCompactWithSchoolTime(c *fiber.Ctx, m *model.StudentClassSection) StudentClassSectionCompactResp {
	resp := FromModelCompact(m)
	normalizeStudentClassSectionCompactTimesToSchoolTime(c, &resp)
	return resp
}

// Compact, slice
func FromModelsStudentClassSectionCompactWithSchoolTime(c *fiber.Ctx, list []model.StudentClassSection) []StudentClassSectionCompactResp {
	out := make([]StudentClassSectionCompactResp, 0, len(list))
	for i := range list {
		r := FromModelCompact(&list[i])
		normalizeStudentClassSectionCompactTimesToSchoolTime(c, &r)
		out = append(out, r)
	}
	return out
}
