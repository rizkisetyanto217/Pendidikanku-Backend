...
// 	// sesi yang sudah lewat <= 15 menit tetap disiapkan (jaga-jaga worker sempat mati)
// 	const graceMins = 15
...
// 		rows, err := tx.Raw(`
// 			SELECT
// 				s.class_attendance_session_id   AS session_id,
// 				s.class_attendance_session_school_id AS school_id
// 			FROM class_attendance_sessions s
// 			WHERE s.class_attendance_session_deleted_at IS NULL
// 			  AND s.class_attendance_session_status IN ('scheduled','ongoing')
// 			  AND s.class_attendance_session_starts_at IS NOT NULL
// 			  AND s.class_attendance_session_starts_at
// 					BETWEEN NOW() - (? * INTERVAL '1 minute')
// 						AND NOW() + (? * INTERVAL '1 minute')
// 			ORDER BY s.class_attendance_session_starts_at
// 			LIMIT ?
