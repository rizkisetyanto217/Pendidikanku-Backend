// file: internals/features/school/sessions/sessions/dto/student_attendance_dto.go
package dto
...
type StudentClassSessionAttendanceURLOpDTO struct {
...
type StudentClassSessionAttendanceCreateRequest struct {
	...
	URLs []StudentClassSessionAttendanceURLOpDTO `json:"urls,omitempty" validate:"omitempty,dive"`
}
...
func (r StudentClassSessionAttendanceCreateRequest) ToModel() attendanceModel.StudentClassSessionAttendanceModel {
	m := attendanceModel.StudentClassSessionAttendanceModel{
		...
		StudentClassSessionAttendanceIsPassed:    r.IsPassed,
		StudentClassSessionAttendanceMarkedAt:    r.MarkedAt,
		StudentClassSessionAttendanceMethod:      r.Method,
		StudentClassSessionAttendanceLat:         r.Lat,
		StudentClassSessionAttendanceLng:         r.Lng,
		StudentClassSessionAttendanceDistanceM:   r.DistanceM,
		StudentClassSessionAttendanceLateSeconds: r.LateSecond,
		StudentClassSessionAttendanceUserNote:    r.UserNote,
		StudentClassSessionAttendanceTeacherNote: r.TeacherNote,
		StudentClassSessionAttendanceLockedAt:    r.LockedAt,
	}
	// status (default: "unmarked" jika nil)
	if r.Status != nil && strings.TrimSpace(*r.Status) != "" {
		m.StudentClassSessionAttendanceStatus = strings.ToLower(strings.TrimSpace(*r.Status))
	} else {
		m.StudentClassSessionAttendanceStatus = "unmarked"
	}
	if r.TypeID != nil {
		m.StudentClassSessionAttendanceTypeID = r.TypeID
	}
	if r.MarkedByTID != nil {
		m.StudentClassSessionAttendanceMarkedByTeacherID = r.MarkedByTID
	}
	return m
}
...
type StudentClassSessionAttendancePatchRequest struct {
...
func (p StudentClassSessionAttendancePatchRequest) ApplyPatch(m *attendanceModel.StudentClassSessionAttendanceModel) error {
	if v, ok := p.Status.Get(); ok {
		if v == nil || strings.TrimSpace(*v) == "" {
			m.StudentClassSessionAttendanceStatus = "unmarked"
		} else {
			m.StudentClassSessionAttendanceStatus = strings.ToLower(strings.TrimSpace(*v))
		}
	}
	if v, ok := p.TypeID.Get(); ok {
		m.StudentClassSessionAttendanceTypeID = v
	}
	if v, ok := p.Desc.Get(); ok {
		m.StudentClassSessionAttendanceDesc = v
	}
	if v, ok := p.Score.Get(); ok {
		m.StudentClassSessionAttendanceScore = v
	}
	if v, ok := p.IsPassed.Get(); ok {
		m.StudentClassSessionAttendanceIsPassed = v
	}
	if v, ok := p.MarkedAt.Get(); ok {
		m.StudentClassSessionAttendanceMarkedAt = v
	}
	if v, ok := p.MarkedByTID.Get(); ok {
		m.StudentClassSessionAttendanceMarkedByTeacherID = v
	}
	if v, ok := p.Method.Get(); ok {
		if v == nil || strings.TrimSpace(*v) == "" {
			m.StudentClassSessionAttendanceMethod = nil
		} else {
			mv := strings.ToLower(strings.TrimSpace(*v))
			m.StudentClassSessionAttendanceMethod = &mv
		}
	}
	if v, ok := p.Lat.Get(); ok {
		m.StudentClassSessionAttendanceLat = v
	}
	if v, ok := p.Lng.Get(); ok {
		m.StudentClassSessionAttendanceLng = v
	}
	if v, ok := p.DistanceM.Get(); ok {
		m.StudentClassSessionAttendanceDistanceM = v
	}
	if v, ok := p.LateSeconds.Get(); ok {
		m.StudentClassSessionAttendanceLateSeconds = v
	}
	if v, ok := p.UserNote.Get(); ok {
		m.StudentClassSessionAttendanceUserNote = v
	}
	if v, ok := p.TeacherNote.Get(); ok {
		m.StudentClassSessionAttendanceTeacherNote = v
	}
	if v, ok := p.LockedAt.Get(); ok {
		m.StudentClassSessionAttendanceLockedAt = v
	}
	return nil
}
...
type ListStudentClassSessionAttendanceQuery struct {
...
type URLMutations struct {
	ToCreate []attendanceModel.StudentClassSessionAttendanceURLModel
	ToUpdate []attendanceModel.StudentClassSessionAttendanceURLModel
	ToDelete []uuid.UUID
}
		...
		case URLOpUpsert:
			if op.ID == nil {
				...
				row := attendanceModel.StudentClassSessionAttendanceURLModel{
					...
					StudentClassSessionAttendanceURLOrder:              pint(op.Order),
					StudentClassSessionAttendanceURLIsPrimary:          pbool(op.IsPrimary),
					StudentClassSessionAttendanceURLHref:               op.Href,
					StudentClassSessionAttendanceURLObjectKey:          op.ObjectKey,
					StudentClassSessionAttendanceURLObjectKeyOld:       op.ObjectKeyOld,
					StudentClassSessionAttendanceURLTrashURL:           op.TrashURL,
					StudentClassSessionAttendanceURLDeletePendingUntil: op.DeletePendingUntil,
					StudentClassSessionAttendanceURLUploaderTeacherID:  op.UploaderTeacherID,
					StudentClassSessionAttendanceURLUploaderStudentID:  op.UploaderStudentID,
				}
				...
				row := attendanceModel.StudentClassSessionAttendanceURLModel{
					StudentClassSessionAttendanceURLID:                 *op.ID,
					StudentClassSessionAttendanceURLLabel:              op.Label,
					StudentClassSessionAttendanceURLOrder:              pint(op.Order),
					StudentClassSessionAttendanceURLIsPrimary:          pbool(op.IsPrimary),
					StudentClassSessionAttendanceURLHref:               op.Href,
				...
				if op.Kind != nil {
					row.StudentClassSessionAttendanceURLKind = kind
				}
