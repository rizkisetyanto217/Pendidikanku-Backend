...
// helper kecil agar WithContext selalu punya ctx saat dipanggil dari helper di atas
func cCtx() func() *fiber.Ctx {
	// no-op placeholder: akan di-override di tiap pemanggilan WithContext di handler.
	// (dipertahankan untuk kompatibilitas—tidak dipakai di handler langsung)
	return func() *fiber.Ctx { return nil }
}
