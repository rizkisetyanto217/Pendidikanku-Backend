...
func (ctrl *EventController) UpdateEvent(c *fiber.Ctx) error {
	...
	if req.EventSchoolID != nil {
		// (Opsional) precheck FK biar error jadi 404/400, bukan 500
		var cnt int64
		if err := ctrl.DB.Table("schools").
			Where("school_id = ?", *req.EventSchoolID).
			Count(&cnt).Error; err != nil {
			return helper.JsonError(c, fiber.StatusInternalServerError, "Gagal memeriksa school")
		}
		if cnt == 0 {
			return helper.JsonError(c, fiber.StatusNotFound, "School tidak ditemukan")
		}
		updates["event_school_id"] = *req.EventSchoolID
	}
...
// 🟢 POST /api/a/events/by-school   (body: { "school_id": "..." }) + pagination
func (ctrl *EventController) GetEventsBySchool(c *fiber.Ctx) error {
	type Request struct {
		SchoolID string `json:"school_id"`
	}
	var body Request
	if err := c.BodyParser(&body); err != nil || body.SchoolID == "" {
	...
	if err := ctrl.DB.Model(&model.EventModel{}).
		Where("event_school_id = ?", body.SchoolID).
		Count(&total).Error; err != nil {
		log.Printf("[ERROR] Count events by school: %v", err)
...
func (ctrl *EventController) DeleteEvent(c *fiber.Ctx) error {
	...
	var ev model.EventModel
	if err := ctrl.DB.
		Where("event_id = ? AND event_school_id = ?", evID, schoolID).
		First(&ev).Error; err != nil {
		return helper.JsonError(c, fiber.StatusNotFound, "Event tidak ditemukan atau bukan milik school ini")
	}
	...
	if hard {
		...
		if err := tx.
			...
			Where("event_session_event_id = ? AND event_session_school_id = ? AND event_session_deleted_at IS NULL", evID, schoolID).
		...
		if err := tx.
			...
			Where(`user_event_registration_event_session_id IN (
                ...
                WHERE event_session_event_id = ? AND event_session_school_id = ? AND event_session_deleted_at IS NOT NULL
            ) AND user_event_registration_deleted_at IS NULL`, evID, schoolID).
