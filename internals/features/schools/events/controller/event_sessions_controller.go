...
// 🟢 GET /api/u/event-sessions/upcoming/:school_id?page=&limit=
// (kalau :school_id kosong, ambil semua publik upcoming)
func (ctrl *EventSessionController) GetUpcomingEventSessions(c *fiber.Ctx) error {
	...
	if schoolIDStr == "" {
		// tanpa school → hanya public
		query = query.Where("event_session_is_public = TRUE")
	} else {
		// filter by school_id
		if _, err := uuid.Parse(schoolIDStr); err != nil {
			log.Printf("[ERROR] invalid school_id: %v", err)
			return helper.JsonError(c, fiber.StatusBadRequest, "Format ID school tidak valid")
		}
		query = query.Where("event_session_school_id = ?", schoolIDStr)
	}
...
func (ctrl *EventSessionController) UpdateEventSession(c *fiber.Ctx) error {
	...
	// Authorization by school
	schoolID, err := helperAuth.GetSchoolIDFromTokenPreferTeacher(c)
	if err != nil || session.EventSessionSchoolID != schoolID {
