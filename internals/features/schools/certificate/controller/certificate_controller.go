...
import (
	...
	lectureExamModel "schoolku_backend/internals/features/schools/lectures/exams/model"
...
// ✅ GET BY USER EXAM ID → detail certificate + lecture + school + grade
func (ctrl *CertificateController) GetByUserExamID(c *fiber.Ctx) error {
	...
	if err := ctrl.DB.First(&school, "school_id = ?", lecture.LectureSchoolID).Error; err != nil {
		return helper.JsonError(c, fiber.StatusNotFound, "School not found")
	}
	...
	response := dto.CertificateDetailResponse{
		...
		// SchoolImageURL:                &school.SchoolImageURL, // pastikan tipe field ini *string di DTO
