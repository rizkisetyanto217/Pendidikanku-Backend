...
func (ctrl *LectureController) CreateLecture(c *fiber.Ctx) error {
	...
	// --- scope school: hanya dari token (prefer teacher -> dkm -> union -> admin) ---
	schoolUUID, err := helperAuth.GetSchoolIDFromTokenPreferTeacher(c)
	if err != nil || schoolUUID == uuid.Nil {
		return helper.JsonError(c, fiber.StatusBadRequest, "School ID tidak valid atau tidak ter-scope")
	}
	...
	if isJSON {
		...
		if isMultipart {
			if fh, err := c.FormFile("lecture_image_url"); err == nil && fh != nil {
				u, upErr := helperOSS.UploadImageToOSSScoped(schoolUUID, "lectures", fh)
				if upErr != nil {
	...
	for i := 2; ; i++ {
		...
		if err := ctrl.DB.
			...
			Where("lecture_slug = ? AND lecture_school_id = ?", slug, schoolUUID).
...
// ✅ GET /api/a/lectures/by-school
func (ctrl *LectureController) GetBySchoolID(c *fiber.Ctx) error {
	schoolID, ok := c.Locals("school_id").(string)
	if !ok || schoolID == "" {
		return helper.JsonError(c, fiber.StatusUnauthorized, "School ID tidak valid atau tidak ditemukan")
	}

	var lectures []model.LectureModel
	if err := ctrl.DB.
		Where("lecture_school_id = ?", schoolID).
	...
	if len(lectures) == 0 {
		return helper.JsonError(c, fiber.StatusNotFound, "Belum ada lecture untuk school ini")
	}
...
func (ctrl *LectureController) UpdateLecture(c *fiber.Ctx) error {
	...
	schoolUUID, err := helperAuth.GetSchoolIDFromTokenPreferTeacher(c)
	if err != nil || schoolUUID == uuid.Nil {
		log.Printf("[LECTURE][%s] ❌ scope fail: school invalid (%v)", reqID, err)
		return helper.JsonError(c, fiber.StatusBadRequest, "School ID tidak valid atau tidak ter-scope")
	}
	log.Printf("[LECTURE][%s] ✅ scope ok school_id=%s", reqID, schoolUUID)
	...
	if existing.LectureSchoolID != schoolUUID {
		log.Printf("[LECTURE][%s] ❌ forbidden: lecture.mosque=%s active=%s", reqID, existing.LectureSchoolID, schoolUUID)
	...
	if isJSON {
		...
		if isMultipart {
			if fh, err := c.FormFile("lecture_image_url"); err == nil && fh != nil {
				...
				url, upErr := helperOSS.UploadImageToOSSScoped(schoolUUID, "lectures", fh)
				if upErr != nil {
	...
	if titleChanged && regenerateSlug {
		...
		newSlug, err := uniqueLectureSlug(ctrl.DB, base, existing.LectureSchoolID, existing.LectureID.String())
		if err != nil {
...
func uniqueLectureSlug(db *gorm.DB, base string, schoolID uuid.UUID, excludeLectureID string) (string, error) {
	slug := base
	var cnt int64
	i := 0
	for {
		q := db.Table("lectures").
			Where("lecture_slug = ? AND lecture_school_id = ?", slug, schoolID)
		if excludeLectureID != "" {
