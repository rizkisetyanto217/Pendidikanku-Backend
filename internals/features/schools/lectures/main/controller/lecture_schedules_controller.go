...
// ✅ Get All (opsional filter ?school_id=...) | GET /api/a/lecture-schedules
// ✅ Get All (with pagination & optional ?school_id=...) | GET /api/a/lecture-schedules
...
// ✅ Get by school_slug | GET /public/lecture-schedules/by-school/:slug
func (ctrl *LectureSchedulesController) GetBySchoolSlug(c *fiber.Ctx) error {
	slug := c.Params("slug")
	if slug == "" {
	...
	err := ctrl.DB.
		Joins("JOIN lectures ON lectures.lecture_id = lecture_schedules.lecture_schedules_lecture_id").
		Where("lectures.lecture_school_id = (SELECT school_id FROM schools WHERE school_slug = ?)", slug).
		Preload("Lecture").
