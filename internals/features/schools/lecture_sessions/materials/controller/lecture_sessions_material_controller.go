...
// =============================
// ➕ Create Lecture Session Material (maksimal 1 per session per school)
// =============================
func (ctrl *LectureSessionsMaterialController) CreateLectureSessionsMaterial(c *fiber.Ctx) error {
	...
	// Ambil school_id dari token/middleware
	schoolID, ok := c.Locals("school_id").(string)
	if !ok || schoolID == "" {
		return resp.JsonError(c, fiber.StatusUnauthorized, "School ID tidak ditemukan dalam token")
	}
	...
	// Cek duplikasi: per (lecture_session_id, school_id) – hanya baris hidup (soft delete otomatis di-skip oleh GORM)
	var existing model.LectureSessionsMaterialModel
	if err := ctrl.DB.WithContext(c.Context()).
		Where("lecture_sessions_material_lecture_session_id = ? AND lecture_sessions_material_school_id = ?",
			body.LectureSessionsMaterialLectureSessionID, schoolID).
	...
	// Simpan
	material := body.ToModel(schoolID)
