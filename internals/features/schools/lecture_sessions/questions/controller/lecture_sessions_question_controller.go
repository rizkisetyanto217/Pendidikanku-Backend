...
import (
	...
	"schoolku_backend/internals/features/schools/lecture_sessions/questions/dto"
...
func (ctrl *LectureSessionsQuestionController) CreateLectureSessionsQuestion(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&bulkBody); err == nil && len(bulkBody) > 0 {
		// Ambil school_id dari token atau dari body[0]
		schoolID := ""
		if raw := c.Locals("school_id"); raw != nil {
			if s, ok := raw.(string); ok {
				schoolID = s
			}
		}
		if schoolID == "" && bulkBody[0].LectureSessionsQuestionSchoolID != "" {
			schoolID = bulkBody[0].LectureSessionsQuestionSchoolID
		}
		if schoolID == "" {
			return resp.JsonError(c, fiber.StatusUnauthorized, "School ID tidak tersedia di token maupun body")
		}
		...
		for i, b := range bulkBody {
			...
			questions = append(questions, model.LectureSessionsQuestionModel{
				...
				LectureSessionsQuestionSchoolID:    schoolID,
			})
		}
	...
	if schoolID == "" && body.LectureSessionsQuestionSchoolID != "" {
		schoolID = body.LectureSessionsQuestionSchoolID
	}
	if schoolID == "" {
		return resp.JsonError(c, fiber.StatusUnauthorized, "School ID tidak tersedia di token maupun body")
	}
