...
func pickUUIDField(v any, names ...string) (uuid.UUID, bool) {
	...
	for _, name := range names {
		...
		// *uuid.UUID pointer
		if f.Kind() == reflect.Ptr && !f.IsNil() && f.Elem().Type() == reflect.TypeOf(uuid.UUID{}) {
			id := f.Elem().Interface().(uuid.UUID)
			if id != uuid.Nil {
				return id, true
			}
		}
...
func pickTimeField(v any, names ...string) (time.Time, bool) {
	rv := reflect.ValueOf(v)
	if rv.Kind() == reflect.Ptr {
		rv = rv.Elem()
	}
	for _, name := range names {
		...
		// *time.Time pointer
		if f.Kind() == reflect.Ptr && !f.IsNil() && f.Elem().Type() == reflect.TypeOf(time.Time{}) {
