...
func (ctrl *ClassAttendanceSessionController) ListBySection(c *fiber.Ctx) error {
	...
	// Query dasar (singular column names)
	qBase := ctrl.DB.Model(&attendanceModel.ClassAttendanceSessionModel{}).
		Where("class_attendance_session_masjid_id = ? AND class_attendance_session_section_id = ?", masjidID, secID)
	...
	// Ambil data (select kolom yang diperlukan saja)
...
func (ctrl *ClassAttendanceSessionController) ListMyTeachingSessions(c *fiber.Ctx) error {
	// Harus teacher (ambil tenant dari klaim teacher)
	...
	parseDate := func(s string) (time.Time, error) {
		// s format YYYY-MM-DD → time.Date di WIB untuk aman
		t, err := time.Parse("2006-01-02", s)
		if err != nil { return time.Time{}, err }
		// treat sebagai tanggal (tanpa jam)
		return time.Date(t.Year(), t.Month(), t.Day(), 0, 0, 0, 0, time.Local), nil
	}

	if df == "" && dt == "" {
		// Default: hari ini s/d besok (inklusif), di zona lokal (WIB).
	...
	// Ambil data (kolom yang diperlukan saja)
	var rows []attendanceModel.ClassAttendanceSessionModel
	if err := qBase.
		Select(`
...
func (ctrl *ClassAttendanceSessionController) ListByMasjid(c *fiber.Ctx) error {
	...
	qBase := ctrl.DB.Model(&attendanceModel.ClassAttendanceSessionModel{}).
		Where("class_attendance_session_masjid_id = ?", targetMasjidID)
	...
	orderClause := "class_attendance_session_date ASC, class_attendance_session_created_at DESC"
	if sort == "desc" {
		orderClause = "class_attendance_session_date DESC, class_attendance_session_created_at DESC"
	}
...
func (ctrl *ClassAttendanceSessionController) CreateClassAttendanceSession(c *fiber.Ctx) error {
	...
	// ===== Transaksi kecil untuk konsistensi =====
	err := ctrl.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 2) Cek duplikat aktif (soft-delete aware)
		var dupeCount int64
		dupeQ := tx.Model(&attendanceModel.ClassAttendanceSessionModel{}).
			Where(`
				class_attendance_session_masjid_id = ?
				AND class_attendance_session_section_id = ?
		...
		// Kalau mau unik per subject juga, buka blok ini
		// if req.ClassAttendanceSessionSubjectId != nil {
		// 	dupeQ = dupeQ.Where("class_attendance_session_subject_id = ?", *req.ClassAttendanceSessionSubjectId)
		// } else {
		// 	dupeQ = dupeQ.Where("class_attendance_session_subject_id IS NULL")
		// }
...
func (ctrl *ClassAttendanceSessionController) UpdateClassAttendanceSession(c *fiber.Ctx) error {
	...
	// Force tenant (tidak boleh pindah masjid)
	req.ClassAttendanceSessionMasjidId = &masjidID
	...
	// Normalisasi tanggal ke DATE (local)
	...
	if err := ctrl.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.
			First(&existing, "class_attendance_session_id = ?", sessionID).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		// 4) Cek unik (section+date) aktif bila berubah
		...
		if err := tx.Model(&attendanceModel.ClassAttendanceSessionModel{}).
			Where(`
				...
				AND class_attendance_session_section_id = ?
				AND class_attendance_session_date = ?
				AND class_attendance_session_id <> ?
				AND class_attendance_session_deleted_at IS NULL
		...
		if req.ClassAttendanceSessionSectionId != nil {
			patch["class_attendance_session_section_id"] = *req.ClassAttendanceSessionSectionId
			existing.ClassAttendanceSessionSectionId = *req.ClassAttendanceSessionSectionId
		}
		patch["class_attendance_session_masjid_id"] = masjidID
		existing.ClassAttendanceSessionMasjidId = masjidID

		if req.ClassAttendanceSessionDate != nil {
			patch["class_attendance_session_date"] = *req.ClassAttendanceSessionDate
			existing.ClassAttendanceSessionDate = *req.ClassAttendanceSessionDate
		}
		if req.ClassAttendanceSessionTitle != nil {
			patch["class_attendance_session_title"] = req.ClassAttendanceSessionTitle
			existing.ClassAttendanceSessionTitle = req.ClassAttendanceSessionTitle
		}
		if req.ClassAttendanceSessionGeneralInfo != nil {
			patch["class_attendance_session_general_info"] = *req.ClassAttendanceSessionGeneralInfo
			existing.ClassAttendanceSessionGeneralInfo = *req.ClassAttendanceSessionGeneralInfo
		}
		if req.ClassAttendanceSessionNote != nil {
			patch["class_attendance_session_note"] = req.ClassAttendanceSessionNote
			existing.ClassAttendanceSessionNote = req.ClassAttendanceSessionNote
		}
		if req.ClassAttendanceSessionTeacherUserId != nil {
			patch["class_attendance_session_teacher_user_id"] = req.ClassAttendanceSessionTeacherUserId
			existing.ClassAttendanceSessionTeacherUserId = req.ClassAttendanceSessionTeacherUserId
		}
		// opsional integrasi subject/csst
		if req.ClassAttendanceSessionSubjectId != nil {
			patch["class_attendance_session_subject_id"] = req.ClassAttendanceSessionSubjectId
			existing.ClassAttendanceSessionSubjectId = req.ClassAttendanceSessionSubjectId
		}
		if req.ClassAttendanceSessionClassSectionSubjectTeacherId != nil {
			patch["class_attendance_session_class_section_subject_teacher_id"] = req.ClassAttendanceSessionClassSectionSubjectTeacherId
			existing.ClassAttendanceSessionClassSectionSubjectTeacherId = req.ClassAttendanceSessionClassSectionSubjectTeacherId
		}

		now := time.Now().In(time.Local)
		patch["class_attendance_session_updated_at"] = now
		existing.ClassAttendanceSessionUpdatedAt = &now

		if err := tx.Model(&attendanceModel.ClassAttendanceSessionModel{}).
			Where("class_attendance_session_id = ?", existing.ClassAttendanceSessionId).
			Select([]string{
				"class_attendance_session_section_id",
				"class_attendance_session_masjid_id",
				"class_attendance_session_date",
...
func (ctrl *ClassAttendanceSessionController) DeleteClassAttendanceSession(c *fiber.Ctx) error {
	...
	if err := ctrl.DB.Transaction(func(tx *gorm.DB) error {
		...
		var delErr error
		if force {
			delErr = tx.Delete(&attendanceModel.ClassAttendanceSessionModel{}, "class_attendance_session_id = ?", sessionID).Error
		} else {
			delErr = tx.Where("class_attendance_session_id = ?", sessionID).
				Delete(&attendanceModel.ClassAttendanceSessionModel{}).Error
		}
		if delErr != nil {
