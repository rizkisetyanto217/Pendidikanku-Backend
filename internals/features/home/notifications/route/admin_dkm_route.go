...
func NotificationAdminRoutes(api fiber.Router, db *gorm.DB) {
	...
	notif.Post("/", notifCtrl.CreateNotification) // buat notifikasi (scoped school)
	...
	nUser.Post("/broadcast", notifUserCtrl.BroadcastToAllUsers)  // broadcast ke semua user (scoped school)
}
