...
func AllPublicRoutes(router fiber.Router, db *gorm.DB) {
	...
	posts.Post("/by-school", postCtrl.GetPostsBySchool) // 🕌 Berdasarkan school_id (body)
	posts.Get("/by-school/:slug", postCtrl.GetPostsBySchoolSlug)
	...
	themes.Post("/by-school", themeCtrl.GetThemesBySchool) // 🕌 Tema by school (body)
}
